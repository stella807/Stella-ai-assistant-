...
| `GET` | `/api/flights/lookup?flightNumber=&date=` | `personal-concierge`. A flight's schedule, status and (while airborne) position — the preview before booking `airport-pickup`, and the same call a booked task's detail screen makes again to refresh. `404` if the provider doesn't recognize the flight, `503` if no provider key is configured — see `docs/fulfillment.md`. |
...
| `POST` | `/api/concierge/tasks` | `personal-concierge`. `{ category, note, location, spendCapCents, acknowledgedDisclosures, assistantId?, quickTask?, hours?, flight? }`. Holds `spendCapCents` plus the category's service fee (discounted when `quickTask` is true and the category is eligible) — see `docs/concierge.md`. The returned task includes `serviceFeeCents` in full. `flight: { flightNumber, date }` is required for, and only for, `category: "airport-pickup"`; the server looks it up itself rather than trusting a client-supplied snapshot — see `docs/fulfillment.md`. |
| `GET` | `/api/concierge/tasks` | — (session; your own tasks only). Every task includes its real `serviceFeeCents` — see `docs/concierge.md`. |
