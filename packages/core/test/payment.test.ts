...
const card = (over: Partial<{ expMonth: number; expYear: number }> = {}) =>
  attachPaymentMethod({ id: "pm1", brand: "Visa", last4: "4242", expMonth: 12, expYear: 2027, now: T0, ...over });
...
describe("attaching a payment method", () => {
  ...
  it("rejects a malformed card number", () => {
    expect(() => attachPaymentMethod({ id: "p", brand: "Visa", last4: "42", expMonth: 1, expYear: 2027, now: T0 })).toThrow();
    expect(() => attachPaymentMethod({ id: "p", brand: "Visa", last4: "abcd", expMonth: 1, expYear: 2027, now: T0 })).toThrow();
  });
  ...
  it("accepts a card expiring later this month", () => {
    ...
    expect(attachPaymentMethod({ id: "p", brand: "Visa", last4: "4242", expMonth: 6, expYear: 2026, now }).expYear).toBe(2026);
  });

  it("requires a brand", () => {
    expect(() => attachPaymentMethod({ id: "p", brand: "  ", last4: "4242", expMonth: 1, expYear: 2027, now: T0 })).toThrow();
  });
